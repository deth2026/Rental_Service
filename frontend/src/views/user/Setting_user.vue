...
// ─── Demo: replace with auth store user ID in production ─────────────────
const USER_ID = 1
...
function validateProfile() {
    if (!profile.value.name.trim()) return 'Full name is required.'
    if (!profile.value.email.trim()) return 'Email address is required.'
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(profile.value.email)) return 'Please enter a valid email address.'
    return null
}
...
async function saveProfile() {
    ...
    try {
        ...
        const res = await userService.updateProfile(USER_ID, fd)
...
function validatePassword() {
    ...
    if (!p.current_password) return 'Current password is required.'
    if (!p.new_password) return 'New password is required.'
    if (p.new_password.length < 8) return 'Password must be at least 8 characters.'
    if (!/[A-Z]/.test(p.new_password)) return 'Password must include an uppercase letter.'
    if (!/[a-z]/.test(p.new_password)) return 'Password must include a lowercase letter.'
    if (!/[0-9]/.test(p.new_password)) return 'Password must include a number.'
    if (!/[^A-Za-z0-9]/.test(p.new_password)) return 'Password must include a special character (!@#$… etc.).'
    if (p.new_password !== p.new_password_confirmation) return 'New passwords do not match.'
    return null
}
...
async function changePassword() {
    ...
    try {
        await userService.changePassword(USER_ID, {
...
<template>
    <div :class="styles['page']">
        ...
        <transition name="toast">
            <div v-if="toast.show" :class="[styles['toast'], styles[`toast--${toast.type}`]]" role="alert">
                <!-- success icon -->
                ...
                <!-- danger icon -->
                <svg v-else :class="styles['toast__icon']" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                ...
                <button :class="styles['toast__close']" @click="toast.show = false" aria-label="Dismiss">
        ...
        <div :class="styles['card']">
            ...
            <div :class="styles['field-group']">
                ...
                <div :class="styles['input-wrap']">
                    ...
                    <input v-model="profile.name" :class="styles['input']" type="text"
            ...
            <div :class="styles['field-group']">
                ...
                <div :class="styles['input-wrap']">
                    ...
                    <input v-model="profile.email" :class="styles['input']" type="email"
        ...
        <div :class="styles['card']">
            ...
            <div :class="styles['field-group']">
                ...
                <div :class="styles['input-wrap']">
                    ...
                    <input v-model="passwordForm.current_password" :class="styles['input']"
