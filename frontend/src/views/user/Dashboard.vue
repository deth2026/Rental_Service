  ...
  <div class="rides-page">
    <div class="rides-shell">
      <header class="topbar">
        ...
        <nav class="nav-links">
          <button v-for="item in navItems" :key="item.label" class="btn-reset nav-link"
            :class="{ active: activeNav === item.label }" @click="setActiveNav(item)">
            {{ item.label }}
          </button>
        </nav>
      ...
      <section class="slideshow-section">
        <div class="containers">
          <div class="slide">
            <article v-for="(slide, index) in orderedSlides" :key="slide.id" class="item" :class="`item-${index + 1}`"
      ...
      <section class="shops-section">
        ...
        <div v-else class="shops-grid">
          <article v-for="shop in visibleShops" :key="shop.id"
            ...
            <div class="shop-card-top">
              <div class="shop-brand-mark">
                <img v-if="shop.ownerAvatar" :src="resolveShopImageUrl(shop.ownerAvatar)"
                  :alt="shop.ownerName || shop.name" class="shop-owner-avatar-img" />
            ...
            <div class="shop-card-details" :class="{ 'shop-card-details--collapsed': !isShopExpanded(shop.id) }">
              ...
              <div class="shop-kpi-row">
                <div class="shop-kpi">
                  ...
                  <strong>⭐️ {{ shop.rating.toFixed(1) }}</strong>
                </div>
                <div class="shop-kpi">
...
:root {
  ...
  --danger: #c2394f;
  --brand: #0ea5e9;
  --shadow-sm: 0 8px 22px rgba(25, 48, 73, 0.08);
  --shadow-md: 0 14px 28px rgba(25, 48, 73, 0.12);
  --shadow-lg: 0 22px 46px rgba(25, 48, 73, 0.17);
}
...
/* Override old brand gradient so new topbar brand shows #2563eb */
/* .topbar .brand {
  color: #2563eb;
  background: none;
  -webkit-text-fill-color: #2563eb;
...
.brand {
  ...
  line-height: 1;
  background: linear-gradient(135deg, #1f9ae5 0%, #0e7fc2 100%);
  -webkit-background-clip: text;
  -webkit-text-fill-color: transparent;
  background-clip: text;
  text-shadow: 0 2px 10px rgba(31, 154, 229, 0.3);
}

.brand span {
  background: linear-gradient(135deg, #2798e7 0%, #0d5ea8 100%);
  -webkit-background-clip: text;
  -webkit-text-fill-color: transparent;
  background-clip: text;
}
...
.top-nav a::before {
  ...
  width: 0;
  height: 2px;
  background: linear-gradient(90deg, #1f9ae5, #0e7fc2);
  border-radius: 2px;
  transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1);
...
.top-nav a.active {
  color: #1f9ae5;
  background: rgba(31, 154, 229, 0.08);
}
...
.booking-popup-btn {
  color: #fff;
  background: #2563eb;
  box-shadow: 0 10px 24px rgba(37, 99, 235, 0.27);
}
...
.search-input:focus,
.filter-select:focus,
.days-controls input:focus {
  outline: 2px solid #9fddcf;
  border-color: #8acebf;
}
...
.shop-card:hover {
  transform: translateY(-2px);
  box-shadow: 0 10px 22px rgba(22, 45, 70, 0.14);
}

.shop-card:nth-child(3n + 1) {
  background: #f8fafc;
  border-color: #dbe3ed;
}

.shop-card:nth-child(3n + 2) {
  background: #f8fafc;
  border-color: #dbe3ed;
}

.shop-card:nth-child(3n + 3) {
  background: #f8fafc;
  border-color: #dbe3ed;
}
...
.shop-card-image::after {
  ...
  position: absolute;
  inset: 0;
  background: linear-gradient(to top, rgba(247, 250, 252, 0.98) 0%, rgba(247, 250, 252, 0.82) 24%, rgba(247, 250, 252, 0) 60%);
  pointer-events: none;
}
...
/* Green dot for active status badge */
.status-green-dot {
  display: inline-block;
  width: 8px;
  height: 8px;
...
.type-pill.active {
  background: linear-gradient(120deg, #0f8d75, #0b6a57);
  color: #fff;
  border-color: #0b6a57;
}
...
.map-wrap {
  ...
  background:
    linear-gradient(120deg, rgba(255, 255, 255, 0.45), rgba(255, 255, 255, 0.2)),
    radial-gradient(circle at 18% 15%, #f7f2c6 0%, transparent 35%),
    radial-gradient(circle at 78% 73%, #d4ece5 0%, transparent 44%),
    #e6f2f6;
}
...
.price-marker {
  ...
  font-size: 0.78rem;
  font-weight: 700;
  box-shadow: 0 7px 15px rgba(20, 42, 51, 0.14);
}
...
.booking-popup-icon {
  ...
  font-size: 1.4rem;
  color: #fff;
  background: linear-gradient(135deg, #21bf79, #0f8d5e);
}
