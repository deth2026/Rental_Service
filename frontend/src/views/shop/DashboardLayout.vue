...
const openEdit = (v) => {
  editId.value = v.id
  Object.assign(form, {
    name: v.name || '', type: v.type || v.category || categories[0] || '', brand: v.brand || '', plate: v.plate || '',
...
const saveVehicle = async () => {
  ...
  const typeValue = String(form.type || form.category || categories[0] || '').trim()
  if (!nameValue || !typeValue || form.price === '' || form.price === null || form.price === undefined) {
